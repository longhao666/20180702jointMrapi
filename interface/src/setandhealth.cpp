...
#define ACC_Z                 0x2C    //加速度计z轴 *1000mg
    ...
    uint16_t data_H = 0;
    // 读取速度
    jointGet(SYS_SPEED_L, 4, (Joint *)m_joint, (void *)&data32, 50, NULL);
    data_L = (uint16_t)(data32 & 0xffff);
    data_H = (uint16_t)(data32>>16);
    float speed = (float) ((data_L + data_H * 65536) * 60.0 / 65536.0 / sys_redu_ratio);
    uiSetAndHealth->speedLineEdit->setText(QString::number(speed, 'f', 2) + "rpm");
    // 读取电流
    jointGet(SYS_CURRENT_L, 4, (Joint *)m_joint, (void *)&data32, 50, NULL);
    data_L = (uint16_t)(data32 & 0xffff);
    data_H = (uint16_t)(data32>>16);
